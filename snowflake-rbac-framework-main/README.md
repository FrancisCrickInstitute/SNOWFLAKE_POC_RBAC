...
|------|--------|----------------|
| sqlfluff-lint | (built-in) | SQL keyword capitalisation, formatting |
| yamllint | (built-in) | YAML syntax in manifest files |
| SQL security lint | `scripts/lint/security_check.sh` | Hardcoded passwords, GRANT TO PUBLIC, GRANT ALL, direct ACCOUNTADMIN usage |
| DCM manifest validation | `scripts/lint/dcm_validate.sh` | manifest.yml YAML syntax, required fields (project_name) |

The `.github/workflows/lint.yml` workflow enforces the same checks on PRs.
